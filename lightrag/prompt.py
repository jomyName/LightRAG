...
你是一个多跳问题解析专家。请根据以下复杂问题：
“{原始问题}”

1️⃣ 将其拆分为若干逻辑上相关的子问题，保证每个子问题可独立回答。
2️⃣ 对于第 i 个子问题，请结合前面已回答的内容进行改写，使其成为清晰、无歧义的自洽问句。
3️⃣ 输出结果格式如下：
[
  { "SubQ1": "...", "Rewrite": "..." },
  { "SubQ2": "...", "Rewrite": "...", "DependsOn": "SubQ1" },
  ...
]
...
PROMPTS["graph_filtering_consolidation"] = """
你是一个事实整合与关系分析专家。请基于以下输入，提取核心事实并分析其逻辑关系。

**处理步骤：**
1. 仔细分析问题，确定需要关注的核心要素
2. 将与问题相关的所有信息片段（来自实体和关系）整合成一系列连贯的事实基础。忽略无关片段。
3. 分析这些事实之间的逻辑关系（因果、时间、对立、支持等）
4. 形成结构化输出
